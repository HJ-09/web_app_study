...
<body>
    ...
    <script>
        function showPopup(){
            //console.log(document.cookie); //⇒ 콘솔로 쿠키 찾기~
            ...
            if(cookiesStr){ //Boolean("",null)  false
                ...
                for(let c of cookies){
                    let cArr=c.split("=");
                    let name=cArr[0].trim(); //trim() 공백지우기
            ...
            //js는 equals가 존재하지 않음. 기본형만 비교가능. 문자열은 자바와 달리 기본형임.
        ...
        <%--    console.log(now.toUTCString());--%>
        <%--    document.cookie =--%>
        <%--        `js_hide_popup=1;expires=${now.toUTCString()};max-age=${60*60*24}`;--%> //max-age는 밀리초 포함 x //쿠키는 라인개행 포함하면 x
        <%--}--%>
        //addCookie();
    </script>
</body>
